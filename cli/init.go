...
var initCmd = &cobra.Command{
  ...
  - Vite/React frontend using @juango/ui
...
func init() {
	initCmd.Flags().StringVarP(&initFlags.module, "module", "m", "", "Go module path (default: github.com/user/<project>)")
...
func runInit(cmd *cobra.Command, args []string) error {
	...
	if modulePath == "" {
		...
		if err == nil && len(user) > 0 {
			modulePath = fmt.Sprintf("github.com/%s/%s", strings.TrimSpace(string(user)), projectName)
		} else {
	...
	if err := npmCmd.Run(); err != nil {
		...
		fmt.Println("You can run 'npm install' manually in the frontend directory")
	}
